...
本ドキュメントは Sumi v0.1.0 を GitHub Release として公開するための手順とチェックリストを定義する。
...
git tag -a v0.1.0 -m "Sumi v0.1.0 - Initial M3 Full MVP release
...
- [ ] 成果物が Release draft に upload されている:
  - `Sumi_0.1.0_x64-setup.exe`（NSIS）
  - `Sumi_0.1.0_x64_en-US.msi`
  - `Sumi_0.1.0_aarch64.dmg`（Apple Silicon）
  - `Sumi_0.1.0_x64.dmg`（Intel Mac）
  - `Sumi_0.1.0_amd64.AppImage`
  - `Sumi_0.1.0_amd64.deb`
...
Sumi v0.1.0 は M3 Full MVP 初回リリースです。日本語話者向けのおしゃれな汎用 Claude Code デスクトップクライアントとして、4 軸の差別化を実装しました。
...
- [ ] 将来 Ed25519 鍵発行時は:
  - `tauri signer generate -w ~/.tauri/sumi.key` で鍵ペア作成
...
All notable changes to Sumi will be documented in this file.
