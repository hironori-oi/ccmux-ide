...
- Organization workflow integration (/ceo /dev /pm etc.)
...
ccmux-ide v0.1.0 は PRJ-012 の M3 Full MVP 初回リリースです。日本語話者 + 組織運営ワークフロー特化の Claude Code デスクトップクライアントとして、4 軸の差別化を実装しました。
...
- [ ] X / Twitter に日本語投稿（claude-code-company と PRJ-012 の文脈含む）
- [ ] 英語版の Show HN 検討（Hacker News）
- [ ] Qiita / Zenn 記事執筆候補（組織運営統合の差別化軸を前面に）
...
- Organization slash commands: `/ceo` `/dev` `/pm` `/research` `/review` `/secretary` `/marketing` `/web-ops`
- `projects/PRJ-XXX/` tree navigation
- `CLAUDE.md` 3-scope (Global / Project / Cwd) inspector with Monaco editor
...
- Organization workflow integration based on [claude-code-company](https://github.com/hironori-oi/claude-code-company).
